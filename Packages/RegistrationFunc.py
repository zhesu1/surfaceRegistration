...
def get_idty_S2(Num_phi, Num_theta):
    phi, theta = torch.linspace(0, np.pi, Num_phi,dtype=torch.double), torch.linspace(0, 2 * np.pi, Num_theta,dtype=torch.double)
...
def compose_gamma(f, gamma):  # f: ...*m*n  gamma: 2*m*n...
    ...
    phi, theta = torch.linspace(0, np.pi, f.size(0),dtype=torch.double), torch.linspace(0, 2 * np.pi, f.size(1),dtype=torch.double)
    PHI, THETA = torch.meshgrid([phi, theta])
